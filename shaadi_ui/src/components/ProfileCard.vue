<template>
  <Card class="profile-card hover:shadow-lg transition-shadow cursor-pointer overflow-hidden" @click="$emit('click', profile)">
    <div class="flex flex-col h-full min-w-0">
      ...
      <div class="relative mb-4">
        ...
        />
        <div v-else class="w-full h-48 bg-gradient-to-br from-purple-100 to-pink-100 rounded-lg flex items-center justify-center">
        ...
        <!-- Verified Badge -->
        <div v-if="profile.verified" class="absolute top-2 left-2 bg-blue-500 text-white px-2 py-1 rounded-full text-xs flex items-center gap-1">
      ...
      <div class="flex-1 min-w-0">
        <h3 class="text-lg font-semibold text-gray-900 mb-1 truncate" :title="profile.full_name">{{ profile.full_name }}</h3>
        <div class="text-sm text-gray-600 space-y-1">
          <div class="flex items-center gap-2">
            <span>{{ profile.age }} years</span>
            <span v-if="profile.height_cm">• {{ formatHeight(profile.height_cm) }}</span>
          </div>
          <div v-if="profile.education" class="truncate" :title="profile.education">{{ profile.education }}</div>
          <div v-if="profile.occupation" class="truncate" :title="profile.occupation">{{ profile.occupation }}</div>
          <div v-if="profile.city" class="flex items-center gap-1 min-w-0">
